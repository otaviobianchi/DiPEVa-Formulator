...
def selected_card(lbl):
    r = T.loc[lbl]
    rows = []
    def get(colkey, title):
        c = meta.get(colkey)
        if c is None: 
            return None
        v = r.get(c, np.nan)
        return (title, v)
    for colkey,title in [("name","Molecule"),("cas","CAS"),("smiles","SMILES/BigSMILES"),("vm","Vm (cm³/mol)"),
                         ("dD","δD"),("dP","δP"),("dH","δH"),("dT","δT"),("sig","σL (mN/m)"),("da","δa")]:
        item = get(colkey,title)
        if item is None: 
            continue
        t,v = item
        if isinstance(v,(float,int,np.floating)) and not np.isnan(v):
...
with tab_form:
    ...
    spec = SETS[purpose]
    mode = st.radio("Mode", ["Pair screening", "PU stoichiometry (NCO/OH)", "Epoxy stoichiometry (EEW/AHEW)", "Ranking"], horizontal=True, key="form_mode")

    A_all = [x for x in spec["A"] if x in T.index]
    B_all = [x for x in spec["B"] if x in T.index]

    if mode == "Pair screening":
        c1, c2 = st.columns(2)
        with c1:
            A = label_select(f"{spec['A_name']} (A)", A_all, key="pair_A")
            selected_card(A)
        with c2:
            B = label_select(f"{spec['B_name']} (B)", B_all, key="pair_B")
            selected_card(B)
        ...
        isos = [x for x in spec["B"] if x in T.index]
        polyols = [x for x in spec.get("subgroups", {}).get("Polyols", spec["A"]) if x in T.index]
        extenders = [x for x in spec.get("subgroups", {}).get("Chain extenders", []) if x in T.index]

        left, right = st.columns(2)
        with left:
            iso = label_select("Isocyanate", isos, key="pu_iso")
            selected_card(iso)
        with right:
            poly = label_select("Main polyol", polyols, key="pu_poly")
            selected_card(poly)

        use_ext = st.checkbox("Add chain extender", value=True, key="pu_use_ext")
        ext = None
        if use_ext and extenders:
            ext = label_select("Chain extender", extenders, key="pu_ext")
            selected_card(ext)
        ...
        auto = st.checkbox("Auto-fill from equivalents library (recommended)", value=True, key="pu_auto")
        ...
        with c1:
            nco_percent = st.number_input("%NCO of isocyanate (wt%)", min_value=0.0, max_value=60.0,
                                          value=nco_default if auto else 33.0, step=0.1, key="pu_nco")
        with c2:
            oh_poly = st.number_input("OH number of polyol (mgKOH/g)", min_value=0.0, max_value=2000.0,
                                      value=oh_poly_default if auto else 56.0, step=1.0, key="pu_oh_poly")
        with c3:
            oh_ext = st.number_input("OH number of extender (mgKOH/g)", min_value=0.0, max_value=2000.0,
                                     ...
                                     step=1.0, key="pu_oh_ext")

        idx = st.number_input("NCO/OH index (ratio of equivalents)", min_value=0.5, max_value=2.5,
                              value=1.05, step=0.01, key="pu_index")

        mass_mode = st.radio("Mass target mode", ["Total mass (includes isocyanate)", "Polyol-side mass (polyol+extender)"], horizontal=True, key="pu_mass_mode")
        total_mass = st.number_input("Target mass (g)", min_value=1.0, value=100.0, step=1.0, key="pu_mass_target")
        ext_frac = 0.0
        if use_ext and ext:
        ...
        resins = [x for x in spec["B"] if x in T.index]
        hardeners = [x for x in spec.get("subgroups", {}).get("Hardeners", spec["A"]) if x in T.index]
        diluents = [x for x in spec.get("subgroups", {}).get("Reactive diluents", []) if x in T.index]

        c1,c2 = st.columns(2)
        with c1:
            resin = label_select("Resin", resins, key="ep_resin")
            selected_card(resin)
        with c2:
            hard = label_select("Hardener", hardeners, key="ep_hard")
            selected_card(hard)

        use_dil = st.checkbox("Add reactive diluent", value=False, key="ep_use_dil")
        dil = None
        if use_dil and diluents:
            dil = label_select("Reactive diluent", diluents, key="ep_dil")
            selected_card(dil)
        ...
        auto = st.checkbox("Auto-fill from equivalents library (recommended)", value=True, key="ep_auto")
        ...
        target_mass = st.number_input("Target mass (g)", min_value=1.0, value=100.0, step=1.0, key="ep_mass_target")
        ...
        if use_dil and dil:
            ...
            EEW_dil = st.number_input("Diluente EEW (g/eq epoxide)", min_value=50.0, max_value=2000.0,
                                      value=EEW_dil_default if (auto) else 150.0, step=1.0, key="ep_EEW_dil")
        ...
        if purpose == "Epoxies":
            resins = [x for x in spec["B"] if x in T.index]
            resin = label_select("Resin (fixed)", resins, key="rk_ep_resin")
            selected_card(resin)
            include_dil = st.checkbox("Rank reactive diluents too", value=True, key="rk_ep_inc_dil")
            ...
            if df.empty:
                ...
                topn = st.slider("Top N", 5, min(100, len(df)), min(20, len(df)), key="rk_ep_topn")
            ...
            isos = [x for x in spec["B"] if x in T.index]
            iso = label_select("Isocyanate (fixed)", isos, key="rk_pu_iso")
            selected_card(iso)
            ...
            if df.empty:
                ...
                topn = st.slider("Top N", 5, min(150, len(df)), min(30, len(df)), key="rk_pu_topn")
                st.dataframe(df.head(topn), use_container_width=True, hide_index=True)
                st.download_button("Download ranking CSV", df.to_csv(index=False).encode("utf-8"),
