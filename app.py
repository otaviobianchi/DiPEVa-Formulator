...
def list_by_class(cls: str) -> list[str]:
    opts = [i for i in T.index if str(T.loc[i, "__class__"]) == cls]
...
def _is_reactive_diluent(ab: str) -> bool:
    # reactive diluent if class says so OR epoxy_fn==1 OR role says diluent
    # BUT never classify difunctional epoxies (e.g., BDGE) as reactive diluents.
    try:
        if _is_epoxy_resin(ab) or _is_epoxy_hardener(ab):
            return False
        r = str(ROLE.get(ab, "")).strip().lower()
        if T.loc[ab, "__class__"] == "reactive_diluent":
            return True
        if "diluent" in r:
            return True
...
with tab_figs:
    ...
    if fam == "Polyesters":
        ...
        dil_sel = st.multiselect("Reactive diluents", ep_dils, default=_def(ep_dils, 12), format_func=label)
