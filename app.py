...
# Preferred database for the app (clean + complete). Falls back gracefully.
...
# =========================
# POLYMERIC FLAG (to keep polyol selector clean)
# =========================
_BIGSMILES_MARKERS = ("{", "}", "[*]", ":[*]", "*]", "repeat", "rep")


def _is_polymeric_like(abbr: str, name: str, smiles_big: str, mw: float) -> bool:
    """Heuristic to identify polymeric (BigSMILES / repeat-unit) entries.

    Used ONLY to filter polyol pools in the UI (does not change chemistry values).
    """
    a = str(abbr or "").upper()
    ...
    # Abbrev cues for polymer families with Mn
    ...
    if any(k in nm for k in [
        ...
        "polyol", "polyethylene glycol", "polypropylene glycol", "polytetramethylene", "caprolactone",
        "polycarbonate diol", "polyether diol", "polyester diol", "ptmeg", "peg", "ppg",
    ]):
        # exclude obvious small-molecule glycols
        if not any(k in nm for k in [
            "ethylene glycol", "diethylene glycol", "triethylene glycol",
            "butanediol", "hexanediol", "propanediol", "glycerol",
        ]):
            return True

    # MW-based fallback (only if naming hints polymer)
...
def list_polyols(polymeric_only: bool = True) -> list[str]:
    """Return polyols; by default only polymeric entries (PEG/PPG/PTMEG/PCL/PCDL...)."""
    opts = list_by_class("polyol")
    if not polymeric_only:
        return opts

    # Lazy-compute flag (works even if DB lacks MW/SMILES/name)
    if "__is_polymeric__" not in T.columns:
        ...
        for ab in T.index:
            ...
            flags.append(bool(_is_polymeric_like(ab, nm, smi, float(mw) if not pd.isna(mw) else float("nan"))))
        T["__is_polymeric__"] = flags

    return [ab for ab in opts if bool(T.loc[ab, "__is_polymeric__"])]
...
with tab_form:
    ...
    # Treat any role containing "epoxy resin" as resin-like (e.g. "epoxy resin (aliphatic)").
    epoxy_resins_role = [ab for ab, r in role_map.items() if "epoxy resin" in r.strip()]
    # Treat a library "diluent" role as a reactive diluent **only if** it is mono-epoxy.
    epoxy_dils_role = [
        ab
        for ab, r in role_map.items()
        if ("diluent" in r)
        and (ab in T.index)
        and (_epoxy_fn(ab) == 1)
    ]
...
with tab_figs:
    ...
    if fam == "Polyesters":
        A = st.multiselect("Polyols", sorted(set(list_polyols(polymeric_only=True) + list_by_class("extender") + list_by_class("crosslinker"))),
